 ...
 * Modified to KOA usage 2/05 2020
 * 添加生成swagger文件功能 2022/03/28
 */
/** @module index */
        ...
        if (title == 'param') {
          let field = parseField(comments[i][j]['name']),
            properties = {
              ...
              default: comments[i][j]['default'] ?? undefined,
            },
...
function generateSpecAndMount(app) {
  return function (options) {
    ...
    const url = options.route ? options.route.url : '/swagger';

    app.use(swaggerUi.serve);
    app.use(convert(mount(url, swaggerUi.setup(swaggerObject))));
    console.log(swaggerObject);
    return swaggerObject;
  };
}

const generateSwaggerFile = (options) => {
  if (!options) {
    throw new Error("'options' is required.");
  ...
  const { type = 'json', name = 'output' } = options.output;
  switch (type?.toLowerCase()) {
    case 'yaml':
      fsPromises.writeFile(`./${name}.yaml`, yaml.dump(swaggerObject));
      break;
    case 'json':
      fsPromises.writeFile(`./${name}.json`, JSON.stringify(swaggerObject, null, 2));
      break;

    default:
      throw new Error("'output.type' must be 'yaml' or 'json'");
  }
};
